...
#source data
whale_consumer <- read.csv("killerwhale_consumer.csv")
whale_disc <- read.csv("killerwhale_discrimination.csv")
whale_sources <- read.csv("killerwhale_sources.csv")


# Put them into the right names
n <- nrow(whale_consumer)
n_isotopes <- 2
K <- nrow(whale_sources)
mu_s <- whale_sources[, c(2, 4)] 
sigma_s <- whale_sources[, c(3, 5)]
mu_c <- whale_disc[, c(2, 4)]
sigma_c <- whale_disc[, c(3, 5)]
